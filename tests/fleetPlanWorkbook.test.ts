...
describe('fleetPlan parser and exporter', () => {
    ...
    it('exports a formatted workbook with the expected sheets, headers, and regenerated totals', async () => {
        ...
        const diesel = workbook.getWorksheet('12m Buses');
        expect(diesel?.getCell('B2').value).toBe('12m Diesel Buses');
        expect(diesel?.getCell('B4').value).toBe('Unit Number');
        expect(getFormula(diesel!.getCell('O10'))).toBe('COUNTIF(O5:O6,"PURCHASE*")');
        expect(getFormula(diesel!.getCell('Q10'))).toBe('COUNTIF(Q5:Q6,"PURCHASE*")');
        expect(getFormula(diesel!.getCell('O12'))).toBe('COUNTIF(O5:O6,"GROWTH")');
        expect(getFormula(diesel!.getCell('O13'))).toBe('O11+O12');
        ...
        expect(getFormula(small!.getCell('J6'))).toBe('COUNT(J5:J5)');

        const electric = workbook.getWorksheet('12m Electric Buses');
        expect(electric?.getCell('B2').value).toBe('Electric 40 Foot Buses');
        expect(electric?.getCell('F5').value).toBe('2501-E');
        expect(electric?.getCell('D8').value).toBe('Total');
        expect(getFormula(electric!.getCell('F8'))).toBe('COUNTIF(F5:F5,"*-E")');
    });

    it('exports a planner-friendly workbook with filters, frozen headers, print setup, and status styling', async () => {
        const buffer = await buildFleetPlanWorkbookBuffer(buildTestFleetPlanWorkbook());
        const workbook = new ExcelJS.Workbook();
        await workbook.xlsx.load(buffer);
        ...
        expect(diesel?.autoFilter).toBe('B4:S6');
        expect(diesel?.views[0]).toMatchObject({
            state: 'frozen',
            ySplit: 4,
            topLeftCell: 'B5',
            showGridLines: false,
        });
        expect(diesel?.properties.tabColor?.argb).toBe('FF2563EB');
        expect(diesel?.pageSetup).toMatchObject({
        ...
        expect(diesel?.headerFooter.oddFooter).toContain('Generated by Transit Scheduler');
        const sheetProtection = (diesel as unknown as { sheetProtection?: { sheet?: boolean; autoFilter?: boolean } }).sheetProtection;
        expect(sheetProtection?.sheet).toBe(true);
        expect(sheetProtection?.autoFilter).toBe(true);
        expect(diesel?.getCell('B5').protection?.locked).toBe(false);
        expect(diesel?.getCell('E5').protection?.locked).toBe(false);
        expect(diesel?.getCell('B4').protection?.locked).not.toBe(false);

        expect(getFillArgb(diesel!.getCell('E5'))).toBe('FFE0F2FE');
        expect(getFillArgb(diesel!.getCell('I5'))).toBe('FFFEE2E2');
        expect(getFillArgb(diesel!.getCell('O6'))).toBe('FFFEF3C7');
        expect(getFillArgb(diesel!.getCell('Q6'))).toBe('FFDCFCE7');
        expect(diesel?.getCell('Q6').dataValidation).toMatchObject({
            type: 'list',
            allowBlank: true,
        });
        expect(getFormula(diesel!.getCell('E3'))).toBe('COUNT(E5:E6)+COUNTIF(E5:E6,"GROWTH")');

        const electric = workbook.getWorksheet('12m Electric Buses');
        expect(electric?.autoFilter).toBe('B4:Q5');
        expect(electric?.pageSetup.printArea).toBe('A1:Q8');
        expect(electric?.properties.tabColor?.argb).toBe('FF059669');
        expect(getFillArgb(electric!.getCell('E5'))).toBe('FFFFD966');
    });
});
