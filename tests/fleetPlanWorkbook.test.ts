...
describe('fleetPlan parser and exporter', () => {
    ...
    it('exports a formatted workbook with the expected sheets, headers, and regenerated totals', async () => {
        ...
        expect(diesel?.getCell('O10').value).toBe(0);
        expect(diesel?.getCell('Q10').value).toBe(1);
        expect(diesel?.getCell('O12').value).toBe(1);
        expect(diesel?.getCell('O13').value).toBe(1);
        ...
        expect(electric?.getCell('F8').value).toBe(1);
    });
});
