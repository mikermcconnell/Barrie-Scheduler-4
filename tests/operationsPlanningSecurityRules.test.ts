...
describe('operations-planning security rules', () => {
  ...
  it('keeps revision JSON immutable for all ordinary members, including managers', () => {
    ...
    const revisionRules = rules.match(
      /match \/teams\/\{teamId\}\/operationsPlanningScenarios\/\{scenarioId\}\/versions\/\{fileName\} \{([\s\S]*?)\/\/ Team route map images/
    )?.[1] ?? '';
    ...
    expect(revisionRules).toMatch(/fileName == request\.resource\.metadata\.revision \+ '\.json'/);
