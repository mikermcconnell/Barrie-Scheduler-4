...
function buildOperatorDwellMetrics(records: STREETSRecord[], date: string): OperatorDwellMetrics {
  ...
  for (const recs of groups.values()) {
    ...
    // Post-midnight guard: treat departure<arrival as rollover only when the gap is large
    // (e.g. 23:58 -> 00:04). Small reverse gaps are timing noise and should be ignored.
    if (depSec < arrSec) {
  ...
  // Compute service hours per operator
  ...
  // Attach normalization to byOperator summaries
  ...
  // System-wide totals use ALL operators' visits/hours, not just those with incidents
