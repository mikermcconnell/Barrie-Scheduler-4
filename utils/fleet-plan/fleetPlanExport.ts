...
const COMBINED_EXPORT_COLUMNS: Array<{ header: string; key: string; width: number }> = [
    { header: 'Bus Type', key: 'busType', width: 18 },
    { header: 'Unit Number', key: 'unitNumber', width: 14 },
    { header: 'Size', key: 'busSize', width: 10 },
    { header: 'Make/Model', key: 'makeModel', width: 24 },
    { header: 'Year', key: 'year', width: 10 },
    { header: 'Comment', key: 'comment', width: 24 },
    { header: 'Electric', key: 'electricFlag', width: 10 },
    { header: 'On Order', key: 'onOrder', width: 16 },
];

const COMBINED_EXPORT_YEARS = Array.from(
...
function renderCombinedFleetPlanSheet(worksheet: ExcelJS.Worksheet, data: FleetPlanWorkbook): void {
    worksheet.properties.tabColor = { argb: 'FF2563EB' };
    worksheet.getCell('A1').value = 'Fleet Plan';
    styleTitle(worksheet.getCell('A1'));
    worksheet.mergeCells(1, 1, 1, COMBINED_EXPORT_COLUMNS.length + COMBINED_EXPORT_YEARS.length);

    const headers = [...COMBINED_EXPORT_COLUMNS.map((column) => column.header), ...COMBINED_EXPORT_YEARS.map((column) => column.label)];
    ...
    [...COMBINED_EXPORT_COLUMNS, ...COMBINED_EXPORT_YEARS.map((column) => ({ header: column.label, key: column.key, width: 11 }))]
    ...
    data.sheets.forEach((sheet) => {
        sheet.rows.forEach((row) => {
            ...
            baseValues.forEach((value, index) => {
                ...
                styleGridCell(cell, coerceFleetExportValue(value), sheet.key);
                if ([0, 3, 5, 7].includes(index)) applyAlignment(cell, 'left');
            });

            COMBINED_EXPORT_YEARS.forEach((column, yearIndex) => {
                const cell = worksheet.getCell(rowNumber, COMBINED_EXPORT_COLUMNS.length + yearIndex + 1);
                styleGridCell(cell, coerceFleetExportValue(row.timeline[column.key] || ''), sheet.key);
                applyStatusValidation(cell);
            });
            rowNumber += 1;
        });
    ...
    const lastColumn = worksheet.getColumn(COMBINED_EXPORT_COLUMNS.length + COMBINED_EXPORT_YEARS.length).letter;
    const lastRow = Math.max(3, rowNumber - 1);
    worksheet.autoFilter = `A3:${lastColumn}${lastRow}`;
    ...
    worksheet.headerFooter.oddFooter = '&LGenerated by Transit Scheduler&RPage &P of &N';
}
...
export async function buildFleetPlanWorkbookBuffer(data: FleetPlanWorkbook): Promise<ArrayBuffer> {
    ...
    const worksheet = workbook.addWorksheet('Fleet Plan');
    renderCombinedFleetPlanSheet(worksheet, data);
