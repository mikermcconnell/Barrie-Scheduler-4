...
Passenger load now belongs in Ridership -> Passenger Flow by Stop; do not restore the standalone Load Profiles tab or its assignable access-control surface. The former tab's versioned monthly read model remains temporarily for backward-compatible API/repair use under `teams/{teamId}/performanceViews/load-profiles/`, published through `loadProfileMonthlyStoragePaths`. Keep `utils/performanceLoadProfileView.ts` and `functions/src/performanceLoadProfileView.ts` contract-identical. Legacy reads go through `sharedWorkspaceData`; do not restore direct browser reads or silently turn load/schema failures into empty data. The endpoint retains its Operations plus Load Profiles checks and bounded route/date validation. Performance publishing remains owner/admin-only, and backfill remains dry-run-first.

Residual access note: the legacy/full monthly performance archives still contain load-profile fields and remain readable to members with the broader Operations workspace because current Ridership analysis consumes those summaries. `operationsLoadProfiles` is retained only for the legacy backend path and stored override compatibility; it is not a supported UI surface or a strict confidentiality boundary for users who already have Operations data access.

Operations metric definitions and validation status live in `docs/OPERATIONS_DASHBOARD_METRICS.md`. Performance schema v10 corrects full-day OTP eligibility for stop/hour rollups, midnight OTP deviation, server/client route-hour and stop-breakdown parity, and exclusion of in-between rows from operational totals. Schema v11 adds reliable stop-level load observation counts for exact multi-day Passenger Flow weighting. Schema v12 adds deterministic dwell-incident IDs, incident operating/passenger context, and route/operator eligible-timepoint exposure for scope-correct Dwell Incident Review rates. Schema v13 adds occurrence-aware load-profile and ridership-heatmap stops so repeated loop visits remain separate while shifted stop positions still align. Schema v14 gives heatmap trips stable identity and stores vehicle/applied capacity so same-time trips do not collide and inference can enforce fleet-specific capacity. Older stored days remain readable but require rebuild or re-import for the newer calculations.
...
- Passenger Flow by Stop uses only positive-source APC departure load first at each stop, then fills only missing or unusable stop loads with an independently calculated heatmap estimate. Do not use daily averaged APC profiles as trip-specific anchors. Group heatmap trips by route plus block, carry `boardings - alightings` through chronological trips, and preserve repeated stop occurrences. Use a zero start when feasible; otherwise use the smallest starting load that keeps the whole block non-negative and disclose it as a lower-bound estimate. Reject ambiguous same-time chains and loads above the trip's applied capacity. Team owners/admins manage the 20-150 passenger default and vehicle overrides at `performanceConfig/load`; 65 is fallback only. Keep inferred points distinct, and keep the opportunity-weighted confidence score plus observed/estimated/historical/unavailable coverage visible. Pre-v14 summaries remain readable but must be rebuilt or re-imported to recover same-time collisions and stable identity.
...
### New Schedule Step 2 runtime trust (July 2026)

- Runtime approvals are schema version 2. `reviewBuckets` are visible evidence; only `approvedBuckets` may generate schedules.
- Performance needs 5 complete paired-cycle days per exact half-hour bucket. The North cycle-start bucket supplies both legs; South-start pairs fail closed. CSV needs an explicit count of 10 observations on every segment.
- Canonical trip matching tolerates harmless raw-stop variations, but missing anchors, short turns, detours, stop-only evidence, estimates, and outliers never become scheduling inputs.
- Strict wizard generation uses the exact approved bucket and throws `MissingApprovedRuntimeError`; do not restore closest-bucket, adjacent-band, raw-segment, or five-minute fallbacks on this path.
- Missing or stale approval blocks later steps, export, and Master upload. Contract readers recompute eligibility instead of trusting stored flags or migration markers.
- Pre-v2 projects are durably reset on load or with `functions/scripts/migrate-new-schedule-runtime-v2.mjs` (dry-run by default). Project saves are revision-checked and serialized; surface stale cross-tab conflicts instead of overwriting them.
