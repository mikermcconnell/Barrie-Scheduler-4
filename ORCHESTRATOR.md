...
- New Schedule / STREETS segment runtime proxies should include intermediate stop dwell for planning realism: measure each observed leg from upstream departure to downstream departure when available, but fall back to downstream arrival at terminal/end-of-trip points so terminal recovery is not baked into segment runtime
...
- Step 2 warning approvals require explicit acknowledgement in the Step 2 UI and persist through the wizard shell state
...
- New Schedule Step 2 in-page approval panel is now status/acknowledgement only; the footer owns the approval/continue action so the gate is not buried inside the page body
