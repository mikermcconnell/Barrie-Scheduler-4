...
### Parking
Owns shared department parking-code usage review. `components/workspaces/ParkingWorkspace.tsx` imports one or more one-month HotSpot Excel workbooks, requires discount-code family mappings before saving, replaces the matching month(s), and derives department month-over-month summaries plus plate-level pattern flags. Department code editability now lives in a modal manager: each department can edit its name, color, short code, active years, year format (`2026` or `26`), manual yearly code overrides, and an `ignoreFlags` option that suppresses plate-level indicators for that department. The Parking settings doc also stores the department color legend sort choice. Matching helpers live in `utils/parking/parkingCodeRules.ts`; flag analysis lives in `utils/parking/parkingAggregation.ts`. Metadata/settings live at `teams/{teamId}/parking/default`; normalized raw rows and summaries live in Storage under `teams/{teamId}/parking/`. Parking contains license plates, so workspace access is limited by default to the `parking`, `admin`, and `internal` profiles; users with Parking workspace access can read, export, import, and maintain Parking settings.
