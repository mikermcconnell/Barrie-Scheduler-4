...
- Add-trip and Extend-trip planners must preserve intermediate recovery/hold metadata across the active span, including deriving recovery from template arrival/departure splits when explicit `recoveryTimes` are missing; copying downstream clock times without that recovery metadata creates operationally dishonest trips
...
Recent durable note:
- Newly added trips should not inherit a delta-source fallback from the template trip they were cloned from. Non-master `+/- Deltas` should only render when a trip has a real original/reference match, not for brand-new added service.
