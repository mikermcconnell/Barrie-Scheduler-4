...
- Route Planner 2 stop-card arrival/kids summaries live in `utils/route-planner-2/routePlanner2StopTimes.ts`; cached Barrie POI search suggestions live in `utils/route-planner-2/routePlanner2PopularPlaces.ts` and are checked before Mapbox for non-civic-address queries.
