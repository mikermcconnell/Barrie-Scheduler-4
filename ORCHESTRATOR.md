...
### Parking
Owns shared department parking-code usage review. `components/workspaces/ParkingWorkspace.tsx` imports one-month HotSpot Excel workbooks, requires discount-code family mappings before saving, replaces the matching month, and derives department month-over-month summaries plus plate-level pattern flags. Logic lives in `utils/parking/`. Metadata/settings live at `teams/{teamId}/parking/default`; normalized raw rows and summaries live in Storage under `teams/{teamId}/parking/`. Parking contains license plates, so workspace access is limited by default to the `parking`, `admin`, and `internal` profiles; writes are owner/admin or global permission manager only.
