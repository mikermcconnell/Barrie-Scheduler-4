...
export const OperatorDwellModule: React.FC<OperatorDwellModuleProps> = ({ data }) => {
    ...
    const dwellDays = new Set(data.dailySummaries.filter(day => day.byOperatorDwell).map(day => day.date)).size;
    const divisor = average ? dwellDays : 1;
    const count = (value: number): string => divisor > 0 ? formatNumber(value / divisor) : '—';
    const label = (text: string): string => average ? `${text} / ${unit}` : text;
    ...
    return (
        <div className="space-y-5">
            ...
            {average && <p className="text-sm text-gray-600">Daily averages use {dwellDays} of {data.dailySummaries.length} selected days with dwell evidence. Incident rows, daily trends, and exports retain actual observations; rates retain their exposure denominator.</p>}

            <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
                <SummaryCard label={label("Reportable incidents")} value={count(model.totalIncidents)} note={`${count(model.reportableDwellMinutes)} reportable dwell minutes${average ? ` / ${unit}` : ""}`} icon={<AlertTriangle size={18} />} tone="amber" />
                <SummaryCard label={label("High severity")} value={count(model.highCount)} note={`${highPct.toFixed(0)}% of reportable incidents`} icon={<Clock3 size={18} />} tone="red" />
                <SummaryCard label={label("OTP-late departures")} value={count(model.otpLateDepartures)} note={`Across ${count(model.otpCarryoverIncidentCount)} originating incidents${average ? ` / ${unit}` : ""}`} icon={<TrendingUp size={18} />} tone="red" />
                ...
                {view === 'incidents' ? (
                    ...
                    <div className="space-y-6 p-4 sm:p-5">
                        ...
                        <section className="rounded-xl border border-gray-200">
                            <div className="border-b border-gray-100 p-4"><h4 className="font-bold text-gray-900">Recurring route, trip, and stop patterns</h4><p className="mt-1 text-sm text-gray-500">Reportable incidents on at least three distinct service days.{average && ` Counts are per ${unit} across all ${dwellDays} days with dwell evidence.`}</p></div>
                        ...
                        <section className="rounded-xl border border-gray-200">
                            ...
                            {model.operatorContext.length === 0 ? <p className="px-4 py-10 text-center text-sm text-gray-500">No operator context is available.</p> : (
                                <><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[700px] text-sm"><thead className="bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500"><tr><th className="px-4 py-3">Operator</th><th className="px-4 py-3 text-right">{label("Eligible visits")}</th><th className="px-4 py-3 text-right">{label("Reportable")}</th><th className="px-4 py-3 text-right">{label("High")}</th><th className="px-4 py-3 text-right">Rate / 1K</th><th className="px-4 py-3 text-right">{label("Reportable dwell")}</th></tr></thead><tbody className="divide-y divide-gray-100">{model.operatorContext.map(operator => <tr key={operator.operatorId}><td className="px-4 py-3 font-mono font-semibold text-gray-900">{operator.operatorId}</td><td className="px-4 py-3 text-right">{operator.eligibleTimepointVisits === null ? '—' : count(operator.eligibleTimepointVisits)}</td><td className="px-4 py-3 text-right">{count(operator.incidentCount)}</td><td className="px-4 py-3 text-right text-red-700">{count(operator.highCount)}</td><td className="px-4 py-3 text-right">{operator.incidentsPer1kEligibleVisits?.toFixed(1) ?? '—'}</td><td className="px-4 py-3 text-right">{divisor > 0 ? minutes(operator.reportableDwellSeconds / divisor) : '—'}</td></tr>)}</tbody></table></div><div className="divide-y divide-gray-100 md:hidden">{model.operatorContext.map(operator => <div key={operator.operatorId} className="p-4 text-sm"><div className="font-mono font-bold text-gray-900">{operator.operatorId}</div><div className="mt-2 text-xs leading-5 text-gray-600">{operator.eligibleTimepointVisits === null ? '—' : count(operator.eligibleTimepointVisits)} {label("eligible visits")} · {count(operator.incidentCount)} {label("reportable")} · {count(operator.highCount)} {label("high")} · {operator.incidentsPer1kEligibleVisits?.toFixed(1) ?? '—'} / 1K · {divisor > 0 ? minutes(operator.reportableDwellSeconds / divisor) : '—'} {label("reportable dwell")}</div></div>)}</div></>
                            )}
                        </section>
                    </div>
                )}
