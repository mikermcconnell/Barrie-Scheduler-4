...
For network redesign work, the right rail should include a compact, visible **Reassign stops** control that can copy or move a contiguous stop range into another route concept. It should require the planner to choose the target route and insertion position instead of guessing where transferred stops belong, and it should offer a planner-controlled reverse-order option when a segment needs to flip direction. Before applying the transfer, show a compact preview with source/target runtime impact, any route-family impact, carried scheduled/manual runtime evidence, connector gaps, duplicate join-stop warnings, and evidence dropped because of reversed direction.
