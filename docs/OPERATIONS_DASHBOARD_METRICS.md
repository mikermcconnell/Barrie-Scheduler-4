...
Scope badges describe the included service days (for example, `5 weekdays selected`) and do not imply that every card is an average. Additive overview metrics follow the shared Sum / Daily average selection; Sum mode also shows a separately labelled average per covered service day. Percentage metrics retain their metric-specific denominator and are not presented as totals or simple averages of daily percentages.
