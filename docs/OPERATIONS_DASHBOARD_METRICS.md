...
| Transit On Demand completed trips | Sum of `Completed Pickups` in each dated Licensee KPI workbook | Presented as a separate system-wide On Demand series in Ridership. It is not added to fixed-route STREETS boardings. The selected service date is user-supplied because the workbook has no report date; pickup and drop-off totals are reconciled and mismatches are disclosed. | Parser, replacement, and selected-period rollup code-validated; operationally confirm the emailed report period before import |
