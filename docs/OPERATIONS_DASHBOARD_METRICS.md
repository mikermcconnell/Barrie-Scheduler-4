...
The Specialized Transit tab is a separate manager-imported source and does not reuse STREETS observations. Its reported trips, common-location subset, recurring-booking indicators, scheduled pickup-time buckets, and endpoint map must remain labelled as Specialized Transit. They must not be added to fixed-route OTP, missed-trip, productivity, or boarding metrics. See `docs/SPECIALIZED_TRANSIT_DASHBOARD.md`.

The dashboard uses one shared time-range and day-type filter across Overview, OTP Analysis, Ridership, Specialized Transit, and Dwell Incident Review. Preset ranges, including Year to Date from January 1 through the latest imported service day, and the inclusive custom start/end range persist when moving between tabs, and every module must apply the same selected dates and day type to its numerator and denominator.
...
- compact tab-specific monthly projection shaping: `utils/performanceDashboardView.ts` and `functions/src/performanceDashboardView.ts`
...
## Shared Sum / Daily average display

The Operations Dashboard owns one `Sum` / `Daily average` selection beside its shared time-range and day-type filters. It defaults to Sum and survives tab switches within the workspace. `utils/performanceAggregation.ts` calculates distinct-date coverage and `components/Performance/performanceAggregation.tsx` supplies the display contract to panels; raw stored summaries are never divided or rewritten.

- Additive period counts in Overview and Ridership (including stop activity, route totals, hourly profiles, passenger-flow boarding/alighting profiles, and trip heatmaps) follow the selection. Average uses distinct included service dates, not calendar days excluded by the day-type filter. Route branches never multiply the day denominator. Heatmaps use the shared date and day-type filters.
- The shared badge states the unit and covered days, including missing calendar dates where the selected window supplies an expected count. Missing reports are not zero activity. A zero-activity report is a covered date. An empty selection displays No data.
- TOD averages use distinct TOD report dates and dwell averages use dates with dwell evidence; both disclose their own coverage. Combined fixed-route/TOD stop activity adds each source's daily average and discloses both denominators. This may be an estimate over different covered date sets.
- Weighted OTP, BPH, load means, peaks, average delay/dwell durations, sample sizes, daily trend points, individual incident/trip records, and raw investigation exports retain their original definitions. Stop Activity Change remains a labelled change per day in both modes.
- Dwell summary, recurring-pattern, and operator-context additive counts/total dwell follow the selection across covered dwell days. Specialized Transit retains its independent monthly-report controls and explicitly states that the shared aggregation does not apply. Legacy computed missed-trip evidence retains an explicit period-total label when a reliable covered-date denominator is unavailable; stored missed-trip evidence uses its own covered dates.
