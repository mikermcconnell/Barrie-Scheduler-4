...
| Transit On Demand activity | Sum of completed pickups or completed drop-offs in the automatically imported daily Licensee KPI workbooks for the active Ridership period | Presented in one separate system-wide map with a pickup/drop-off toggle, imported-day count, period, and selected metric total. It is not added to fixed-route STREETS boardings. Power Automate supplies the previous Toronto calendar day because the workbook has no trustworthy report date; pickup and drop-off totals remain independently visible. | Parser, automatic replacement, selected-period aggregation, and metric-toggle behavior code-validated; monitor the email flow and imported service date operationally |
