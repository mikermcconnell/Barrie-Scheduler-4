...
`teams/{teamId}/strategicPlanWorkplans/default` stores the current editable project-control schedule for the Strategic Plan team. Schema v1 contains a bounded source record, schedule range, monotonic `revision`, and at most 250 tasks with WBS, phase/chapter, ownership, weekly dates, status, progress, dependencies, notes, and typed task/deliverable/review/meeting/engagement segments. Each save checks the loaded revision in a transaction and writes an immutable full snapshot to `strategicPlanWorkplans/default/versions/{revision}`. Members with `analyticsStrategicPlan` may read and maintain their current team's work plan; authenticated developer support retains its scoped read/edit behavior. A team's evidence-source configuration does not redirect or share this project-control document. The work plan contains planning metadata only and does not grant write access to any source evidence, Master Schedule, Fleet Plan, GTFS, or performance record.
