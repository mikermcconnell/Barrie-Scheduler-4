...
The home selector may expose permission-filtered quick links into these shells. Operations Dashboard tabs use nested hashes beneath `#operations/performance`; for example, `#operations/performance/ridership` opens the Ridership tab, while `#operations/performance/specialized-transit` opens Specialized Transit independently of STREETS metadata and overview availability. Tab changes stay synchronized with browser history. Specialized Transit currently shares the Operations workspace access boundary, with manager-only import/review controls.
...
The Operations landing path reads metadata before requesting its compact overview payload. The system overview renders its dataset context, KPIs, action queue, and route scorecard before loading the Recharts-backed visualizations in `SystemOverviewCharts.tsx`, keeping chart parsing and layout out of the first useful paint. Historical dashboard requests prefer immutable monthly projections for the active Overview, OTP, Ridership, or Dwell tab; the projections omit unrelated tab fields and schedule-runtime evidence while the full monthly archive remains available to scheduling consumers. A projection is used only when it covers every requested source month, so partial generations fall back instead of returning incomplete history. Same-team browser reads cap Storage downloads at four concurrent files. Shared-source reads also use at most four concurrent requests, but request one month per backend response so multi-month Ridership and Dwell payloads stay below the Cloud Functions response-size boundary before being merged client-side. Dashboard loading feedback reports completed monthly files when that progress is available and otherwise identifies the active loading stage. After a 500 ms anti-flicker delay, it may show an approximate remaining time learned from the five most recent successful loads in that browser. Timing profiles are anonymous request classes stored only in local storage; failed or superseded requests are not recorded, and an expired estimate changes to a longer-than-usual message instead of reaching zero while loading continues.
...
The Stop Activity Map has an optional Property lines overlay. `utils/barrieParcelBoundaries.ts` queries the City of Barrie's public Open Data parcel service for polygon geometry in the visible map area, beginning at zoom 16, and pages responses when necessary. The map renders outlines behind its route and stop layers, retains loaded geometry only in browser memory, and does not use parcel attributes or change ridership measures. A map-local minimum-activity filter supports a numeric threshold and 20+ preset, filters searchable/rendered stops, and reports the match count; in change mode it filters by absolute change per day. The map also toggles between light and satellite basemaps, increasing parcel-line contrast on satellite imagery. These controls affect map presentation only and do not change stored ridership data.

TOD zone assignment is a separate planning layer within that same activity card. `utils/todZones/` owns effective-version selection, boundary-inclusive point-in-polygon assignment, explicit per-zone connection-stop membership, explicit stop overrides, City GIS stop loading, GeoJSON interchange, the permanent Zone A-F/H and temporary Zone T seeds, the one-time mutable-draft schema migration, and Firestore persistence. `TodZoneEditor.tsx` is the manager-only fullscreen drawing and validation surface; `TodActivityMap.tsx` renders the applicable source-colored published polygons and single/shared connection-stop symbols below activity points and exposes zone, multi-zone, and unassigned filters. Each selected service date is classified with its effective published version, while the visible overlay uses the latest applicable version and discloses when a period spans multiple versions. Zone grouping never creates route, hourly, comparison, or additive ridership evidence that the source does not contain. The durable behavior contract is `docs/TOD_ZONES.md`.
...
- `components/Parking/ParkingStrategyWorkspace.tsx`
- `utils/parking/`

`ParkingWorkspace.tsx` is the lightweight card-dashboard and routing shell. It does not read Parking data or load the chart, map, workbook-parser, or export implementation. The dashboard exposes Plate Monitor and one combined Parking Lot Data entry. `ParkingLotWorkspaceShell.tsx` gives Parking Lot Data a shared header and HotSpot & QR / LocoMobi History source tabs. Only the active source child mounts. Selecting Plate Monitor or the HotSpot & QR tab lazy-loads `ParkingDataWorkspace.tsx`; Plate Monitor downloads only the department-code payload, while HotSpot & QR downloads both department-code and revenue payloads because it exposes both analyses. Workbook parsing and export dependencies load only when the planner imports or exports a file.

Historical LocoMobi strategy evidence is isolated from those composite raw payloads. The Parking shell lazy-loads `ParkingStrategyWorkspace.tsx` inside the LocoMobi History tab for Trends and Graphs at `#parking/lot-data/history` and its connected Map Analysis at `#parking/lot-data/history/map`, which is the initial source-tab destination. Legacy `#parking/strategy` and `/map` bookmarks remain supported with their context. The shared source tabs remember each source's last visited URL within the shell; source-specific calculations, imports, exports and data stores remain separate. `parkingLocoMobiParser.ts` selects the canonical Worldstream table, deduplicates before privacy projection, and emits only non-identifying payment activity. `parkingLocoMobiAggregation.ts` builds the aggregate read model, including a compact `locationMonths` index for period/location cross-filtering. `parkingStrategyHistoryService.ts` stores one active manifest at `teams/{teamId}/parking/history`, loads the aggregate first, and exposes privacy-minimized month partitions only for explicit drill-down. Older aggregates without the index remain readable but require re-import for linked period filters. The standalone HTML concepts under `output/parking-strategy-mockups/` remain design references.

`parkingStrategyModel.ts` groups source meters by reviewed physical-location links, falling back to source domains for unlinked meters. `parkingStrategyLocationService.ts` stores revisioned links in `teams/{teamId}/parking/historyLocations`, referencing the existing reviewed location registry without changing `parking/default` or its HotSpot/QR mappings. `ParkingStrategyMap.tsx` reuses Mapbox `MapBase` and displays only linked physical locations with valid reviewed coordinates; unmatched records stay in evidence totals without invented pins, and distinct lots are not distance-clustered. `parkingStrategyRouting.ts` carries `from`, `to`, and `area` between board and map. Links to Parking Lot Data add the reviewed `location` ID and `origin=strategy`; that workspace applies the carried calendar-month range to its map, revenue trends, and capacity trends, while retaining its own tax-inclusive metric semantics. The visible strategy context can be cleared, ordinary year/month controls override it, and a return link restores the original strategy selection. Missing uploaded periods never widen to all revenue history. LocoMobi source-reported amounts remain separate from HotSpot/QR revenue.

The Parking workspace opens to a card dashboard, similar to Fixed Route Operations, with separate Plate Monitor and Parking Lot Data workspaces. Parking Lot Data is a Route Planner-style full-screen map-first shell for Parking Revenue analytics: it imports and auto-saves HotSpot app and QR revenue workbooks as source-aware monthly datasets, ships with bundled City ParkingLatLong lot coordinates/spaces as default reviewed map locations, stores normalized row-level data in team Storage, keeps reviewed source-ID mappings and map locations in Firestore settings, uses public City of Barrie parking GIS locations as map-only fallback coordinates until reviewed locations are saved, and summarizes revenue, sessions, stay length, peak periods, location comparisons, category comparisons across the requested operating metrics, and estimated capacity utilization with monthly trend views. `parkingLocationWorkbook.ts` can parse a newer City lat/lng workbook, but no production UI currently invokes it; do not describe location-workbook refresh as a shipped user action until that reviewed import flow is wired. Parking staff can maintain editable revenue category labels in settings; seeded categories are Downtown, Waterfront, Hybrid, Marina, Hospital, Allandale GO, and Special Events. A revenue mapping can be a physical parking location or an intentionally non-spatial group. HotSpot and QR ID `9000` are seeded as the same non-spatial Special Events group: those rows participate in revenue, session, category, source, and time/trend analysis, but the group has no map pin and is excluded from capacity-derived metrics and spatial map-coverage denominators. Year/month/category/uploader/day/source/hour filters drive both map and analysis views, and single-month trend views switch to daily trend data. Map pins may group multiple source IDs for the same physical lot, but distinct nearby lots or on-street parking areas must remain separate rather than distance-clustered. Public-source matches are treated as normal map pins in the UI rather than user-facing unmapped warnings. The older department-code usage import remains in Parking Lot Data for department summaries. Plate Monitor reviews the derived plate-level pattern flags and indicator thresholds; its annual department matrix shows both discount value and use count, and every value, including monthly and annual totals, opens the matching raw observations for Excel or PDF export.
...
Visible workspace access is controlled by `utils/workspaceAccess.ts`. Global feature flags decide whether a feature exists in the build; user/team workspace access decides whether the current user can see and enter it. Signed-in ordinary users without a team member record, or with the `none` profile, see only the blocking Team Management setup flow until access is granted. A `schedulerAdmin` account can enter Team Management without joining a home team and automatically receives the `internal` workspace profile when it does belong to a home team. Developer support uses one expiring team-scoped session for other teams: `inspect` grants cross-team reads, while `edit` grants cross-team writes for up to 60 minutes and requires an audited reason. The selected team/member surface changes only in `useTeam()` and never rewrites `users/{uid}.teamId`. Team Management's Data Sources tab can point partner teams at separate Transit App, STREETS, Fleet Plan, Master Schedule, and Strategic Plan work-plan sources through `teams/{teamId}.dataSourceTeamIds`; aggregate Transit App/STREETS reads use `sharedWorkspaceData`, Master Schedule reads use the existing Firestore/Storage shared-source rules, and the work-plan mapping grants explicitly editable collaboration only within the revisioned Strategic Plan work plan. External agency teams should be onboarded through the partner-team invite-link flow; use the `2027–2032 Strategic Plan only` package for embedded strategic evidence and work-plan collaboration, `Transit App Data only` when a team should see only the standalone Transit App workspace, or `Transit App + STREETS Dashboard` for WATT-style access.

Notable areas:
- Specialized Transit → `components/Performance/SpecializedTransitModule.tsx`, `components/Performance/SpecializedTransitMap.tsx`, and `utils/specialized-transit/`; client-only PDF parsing produces a privacy-minimized aggregate independent of the STREETS archive. `locationResolver.ts` owns canonical Barrie matches, Permanent Mapbox requests, civic-number/semantic candidate scoring, automatic collision rejection, and inputs to collapsed location-review tools. `locationMerges.ts` preserves manager alias merges across imports and consolidates RVH campus activity on read and subsequent saves. The map owns fullscreen, ranked Top/All tables, and location selection/focus. `permanentGeocodingBudget.ts` claims each billable request before dispatch and enforces the browser-local 999-request monthly ceiling. Vite localhost persists the aggregate in IndexedDB; optional source PDF pairs are browser-local and keyed by user/team on all hosts. Non-local aggregates use team-readable, manager-writable revisioned Firebase persistence. The durable source, interpretation, access, and location-review contract is `docs/SPECIALIZED_TRANSIT_DASHBOARD.md`.
- Ridership Trends → `components/Analytics/RidershipTrendsWorkspace.tsx`, `utils/ridership-trends/`; a Friendly Design, graph-led Planning Data view that combines a generated workbook baseline through July 2026 with a compact long-lived daily STREETS boarding projection from August 2026 onward. Automatic and manual performance publishers replace the projection pointer with the rest of the performance generation so detailed 380-day retention cannot erase annual history. `functions/src/monthlyRidershipReport.ts` reuses that compact projection plus the narrow On Demand projection to send the previous month's Barrie-blue HTML report at 10:00 Toronto time on the first day. It queues one deterministic Firestore email and audit record per month, while `monthlyRidershipReportHtml.ts` owns the email-safe layout and inline chart. The durable contract is `docs/RIDERSHIP_TRENDS.md`.
...
- 2027–2032 Strategic Plan → `components/Analytics/StrategicPlanWorkspace.tsx`, `components/Analytics/StrategicWorkplanWorkspace.tsx`, `utils/strategic-plan/`; a visually distinct project-control workspace plus a card-led evidence library. The work plan is seeded from the week-precision Dillon proposal schedule, opens to Full Schedule with complete task editing in a drawer, persists one optimistic-revision document and immutable field-audited save versions in Firestore, and retains Update Desk and Timeline views over the same task model. `dataSourceTeamIds.strategicPlanWorkplan` lets a permitted Dillon team collaborate in Barrie's canonical work plan without granting writes to evidence sources. Evidence workspaces expose the read-only static-GTFS baseline, complete canonical Transit App aggregate, current canonical Fleet Plan, and canonical published Master Schedule. `hooks/useTransitAppData.ts`, `TransitAppAnalysisView`, `fleetPlanService`, and a read-only `MasterScheduleBrowser` are reused, so the plan creates no duplicate evidence dataset, workbook, or schedule snapshot.
...
membership. `utils/run-cutting/interiorRelief.ts` resolves source stop events
and projects version-2 operator intervals into private exact-coverage work units
for the existing duty calculator and validator. External proposals retain
original Master trip/block identities and event references; the projection
never calls a publish or block-assignment service. Firestore stores bounded workflow metadata; immutable scenario
