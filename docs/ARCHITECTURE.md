...
Transit On Demand activity remains separate from fixed-route ridership within the Ridership tab. `functions/src/todDailyKpi.ts` owns authenticated email auto-ingest, workbook parsing, raw-workbook archival, read-before-write protection, and optimistic active-pointer replacement. Power Automate supplies the previous Toronto calendar day because the workbook contains no trustworthy service date. The client reads those daily reports through `utils/todPickupService.ts`, aggregates only the Ridership period's included service dates, and renders one map-focused card with a pickup/drop-off toggle. Manual daily workbook and legacy monthly CSV uploads are no longer exposed; retained monthly datasets remain read-only compatibility data and are not rendered. On Demand activity is never added to fixed-route STREETS boardings or attributed to fixed routes.
