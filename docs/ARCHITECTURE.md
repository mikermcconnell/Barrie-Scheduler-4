...
Performance planning canonicalizes complete trips onto the official stop chain before same-day North/South pairing. Raw stop-pattern variations do not disqualify an otherwise complete trip. The North cycle-start half-hour bucket supplies both paired legs; a South-start performance pair fails closed because no trustworthy cycle-start mapping exists. CSV percentiles become eligible only when the upload includes explicit per-segment observation counts. Stop-level, count-free CSV, and detour evidence remain troubleshooting-only.
