...
Transit On Demand ridership remains a separate system-wide series within the Ridership tab. `utils/todDailyKpiParser.ts` reads the daily Licensee KPI Top Locations workbook for both manual and server imports, while `utils/todPickupService.ts` stores manual dated reports alongside the existing monthly pickup-map summary. `functions/src/todDailyKpi.ts` owns authenticated email auto-ingest, raw workbook archival, read-before-write protection, and optimistic active-pointer replacement. Power Automate supplies the previous Toronto calendar day because the workbook contains no date. The UI applies the Ridership period's included service dates, but does not add On Demand completed trips to fixed-route STREETS boardings or imply route-level attribution.
