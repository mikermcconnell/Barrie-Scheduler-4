...
The Stop Activity Map has an optional Property lines overlay. `utils/barrieParcelBoundaries.ts` queries the City of Barrie's public Open Data parcel service for polygon geometry in the visible map area, beginning at zoom 16, and pages responses when necessary. The map renders outlines behind its route and stop layers, retains loaded geometry only in browser memory, and does not use parcel attributes or change ridership measures.
